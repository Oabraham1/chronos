 ...
 *
 * This file contains unit tests for the ChronosPartitioner class, ensuring
 * that it correctly creates, lists, and releases partitions, as well as
 * handles automatic expiration and error conditions.
 *
...
int testPartitionExpiration() {
  try {
    ...
    std::cerr << "Exception in testPartitionExpiration: " << e.what()
              << std::endl;
    return 1;
  }
}
  ...
  try {
    ...
    for (int i = 0; i < numPartitions; i++) {
      ...
      if (partitionId.empty()) {
        ...
        std::cout << "Created " << i
                  << " partitions before running out of memory" << std::endl;
        break;
      }
    ...
    std::cerr << "Exception in testMultiplePartitions: " << e.what()
              << std::endl;
    return 1;
  }
}
