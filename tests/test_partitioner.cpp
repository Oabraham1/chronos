...
int testPartitionBasic()
{
    try
    {
        ...
        {
            std::cout << "Could not create partition (possibly no OpenCL devices available) - skipping test in CI environment" << std::endl;
    ...
    try
    {
        ...
        for (int i = 0; i < numPartitions; i++)
        {
            ...
            if (partitionId.empty())
            {
                ...
                std::cout << "Created " << i << " partitions before running out of memory" << std::endl;
                break;
            }
