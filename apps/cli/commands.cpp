...
namespace chronos
{
    namespace cli
    {
        ...
        int executeCreate(ChronosPartitioner &partitioner, int argc, char *argv[])
        {
            if (argc != 5)
            {
                std::cerr << "Error: 'create' command requires device index, memory fraction, and duration" << std::endl;
            ...
            try
            {
                ...
                {
                    std::cerr << "Error: memory fraction must be between 0 and 1" << std::endl;
                    return 1;
                }
                ...
                std::string partitionId = partitioner.createPartition(deviceIdx, memoryFraction, duration);
        ...
        int executeAvailable(ChronosPartitioner &partitioner, int argc, char *argv[])
        {
            if (argc != 3)
            {
                std::cerr << "Error: 'available' command requires device index" << std::endl;
                ChronosUtils::printUsage();
                return 1;
            }
            ...
            try
            {
                ...
                {
                    std::cout << std::fixed << std::setprecision(2) << availablePercent << std::endl;
                    return 0;
                }
                else
                {
            ...
            {
                std::cerr << "Error getting available percentage: " << e.what() << std::endl;
                return 1;
            }
        }
