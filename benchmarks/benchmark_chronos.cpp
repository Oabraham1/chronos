 ...
 *
 * This benchmark suite evaluates the performance of the Chronos system
 * across several key dimensions critical for academic evaluation. It measures:
 * - Partition creation/release latency: The overhead of the core API calls.
 * - Scalability: System performance as the number of concurrent partitions
 * increases.
 * - Expiration timing accuracy: The precision of the time-based resource
 * reclamation.
 *
...
public:
  ...
  BenchmarkResult benchmarkExpirationAccuracy(int iterations = 10) {
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      std::string partitionId =
          partitioner.createPartition(0, 0.1f, expected_duration_s);
      ...
      if (!partitionId.empty()) {
        ...
        double error_ms =
            (actual_duration.count() - expected_duration_s) * 1000.0;
  ...
  void runAllBenchmarks(
      const std::string &outputFile = "chronos_benchmark_results.csv") {
    std::cout << "=== Chronos GPU Partitioner Benchmark Suite ===" << std::endl;
    std::cout << "Running comprehensive performance evaluation..." << std::endl
              << std::endl;
...
private:
  ...
  BenchmarkResult calculateStats(const std::string &name,
    ...
    result.stddev_ms =
        std::sqrt(sq_sum / values.size() - result.mean_ms * result.mean_ms);
  ...
  void printResults(const std::vector<BenchmarkResult> &results) {
    ...
    std::cout << std::left << std::setw(30) << "Test Name" << std::right
              << std::setw(15) << "Mean (ms)" << std::setw(15) << "StdDev (ms)"
              << std::setw(15) << "Min (ms)" << std::setw(15) << "Max (ms)"
    ...
    for (const auto &r : results) {
      std::cout << std::left << std::setw(30) << r.name << std::right
                ...
                << r.mean_ms << std::setw(15) << r.stddev_ms << std::setw(15)
                << r.min_ms << std::setw(15) << r.max_ms << std::setw(10)
  ...
  void saveResultsToCSV(const std::vector<BenchmarkResult> &results,
    ...
    file << "Test Name,Mean (ms),StdDev (ms),Min (ms),Max (ms),Samples"
         << std::endl;

    for (const auto &r : results) {
      file << r.name << "," << r.mean_ms << "," << r.stddev_ms << ","
           << r.min_ms << "," << r.max_ms << "," << r.iterations << std::endl;
    }
