 ...
 * - Using multiple partitions across different GPUs
 * - Handling partition failure gracefully
 * - Using OpenCL with the partitions
 */
...
public:
    ...
    bool initialize()
    {
            ...
            try
            {
                ...
                for (int deviceIdx = 0; deviceIdx < 2; deviceIdx++)
                {
                    ...
                    if (!partitionId.empty())
                    {
                        ...
                        auto task = std::make_unique<GpuTask>(deviceIdx, partitionId, memFraction, vectorSize);
        ...
        d_c = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
                             sizeof(float) * vectorSize, nullptr, &err);
        if (err != CL_SUCCESS)
        {
...
private:
    ...
    void runTask(int iterations)
    {
        ...
        for (int iter = 0; iter < iterations && running; iter++)
        {
            ...
            {
                err = clEnqueueReadBuffer(queue, d_c, CL_TRUE, 0, sizeof(float) * vectorSize, h_c.data(), 0, nullptr, nullptr);
                if (err != CL_SUCCESS)
                {
                ...
                printMessage("Iteration " + std::to_string(iter) + ": " + (valid ? "Valid" : "Invalid") + " results");
            }
    ...
    void printError(const std::string &message)
    {
        ...
        std::cerr << "[GPU " << deviceIdx << ", Partition " << partitionId << "] ERROR: " << message << std::endl;
    }
    ...
    {
        std::lock_guard<std::mutex> lock(consoleMutex);
        std::cout << "[GPU " << deviceIdx << ", Partition " << partitionId << "] " << message << std::endl;
    }
