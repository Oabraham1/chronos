...
namespace chronos
{
    ...
    std::string ChronosPartitioner::createPartition(int deviceIdx, float memoryFraction, int durationInSeconds)
    {
    ...
    void ChronosPartitioner::showDeviceStats()
    {
        pImpl->showDeviceStats();
    }
        ...
        {
            {
                ...
                for (auto &partition : partitions)
                {
                    if (partition.active)
                    {
                        auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - partition.startTime);

                        if (elapsed >= partition.duration)
                        {
                            releasePartitionResources(partition);
                            partition.active = false;
                            std::cout << "Partition " << partition.partitionId << " expired and released" << std::endl;
                        }
                    }
                }

                auto it = std::remove_if(partitions.begin(), partitions.end(),
                                         [](const core::GPUPartition &p)
                                         { return !p.active; });
    ...
    void ChronosPartitioner::Impl::releasePartitionResources(core::GPUPartition &partition)
    {
        for (auto &device : devices)
        {
            if (device.id == partition.deviceId)
            {
                ...
                lockFile.releaseLock(getDeviceIndex(partition.deviceId), partition.memoryFraction);
                break;
            }
        }
    }
    ...
    bool ChronosPartitioner::Impl::canAccessGPU(int deviceIdx, float memoryFraction)
    {
        ...
        if (requestedMemory > devices[deviceIdx].availableMemory)
        {
            ...
            std::cerr << "Requested: " << (requestedMemory / (1024 * 1024)) << " MB, "
                      << "Available: " << (devices[deviceIdx].availableMemory / (1024 * 1024)) << " MB" << std::endl;
            return false;
        }
    ...
    std::string ChronosPartitioner::Impl::createPartition(int deviceIdx, float memoryFraction, int durationInSeconds)
    {
        std::lock_guard<std::mutex> lock(partitionMutex);
        ...
        if (requestedMemory > device.availableMemory)
        {
            std::cerr << "Not enough available memory on device " << deviceIdx << std::endl;
            std::cerr << "Requested: " << (requestedMemory / (1024 * 1024)) << " MB, "
                      << "Available: " << (device.availableMemory / (1024 * 1024)) << " MB" << std::endl;
            return "";
        }
    ...
    std::vector<core::GPUPartition> ChronosPartitioner::Impl::listPartitions(bool printOutput)
    {
        ...
        if (printOutput)
        {
            ...
            for (const auto &partition : activePartitions)
            {
                ...
                std::cout << "  Owner: " << partition.username << " (PID: " << partition.processId << ")" << std::endl;
                std::cout << std::endl;
            }
        }
    ...
    bool ChronosPartitioner::Impl::releasePartition(const std::string &partitionId)
    {
        ...
        for (auto &partition : partitions)
        {
            if (partition.partitionId == partitionId && partition.active)
            {
                ...
                {
                    std::cerr << "Permission denied: partition owned by " << partition.username << std::endl;
                    return false;
                }
    ...
    void ChronosPartitioner::Impl::showDeviceStats()
    {
        ...
        for (size_t i = 0; i < devices.size(); i++)
        {
            ...
            float memoryUsagePercent = 100.0f * (1.0f - (float)device.availableMemory / device.totalMemory);
            ...
            std::cout << "    Used: " << ((device.totalMemory - device.availableMemory) / (1024 * 1024)) << " MB" << std::endl;
            std::cout << "    Available: " << (device.availableMemory / (1024 * 1024)) << " MB" << std::endl;
            std::cout << "    Usage: " << std::fixed << std::setprecision(2) << memoryUsagePercent << "%" << std::endl;
