...
std::string LockFile::generateLockFilePath(int deviceIdx,
                                           float memoryFraction) const {
  int memPercent = static_cast<int>(std::round(memoryFraction * 1000));

  std::stringstream ss;
  ss << pImpl->basePath << "gpu_" << deviceIdx << "_" << std::setw(4)
     << std::setfill('0') << memPercent << ".lock";

  return ss.str();
}

bool LockFile::createLock(int deviceIdx, float memoryFraction,
...
std::string LockFile::getLockOwner(int deviceIdx, float memoryFraction) const {
  ...
  std::string lockFilePath = generateLockFilePath(deviceIdx, memoryFraction);
  std::string content =
      platform::Platform::getInstance()->readFile(lockFilePath);
